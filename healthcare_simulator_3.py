...
sigma = st.number_input("Log-Std Dev (σ)", value=1.0, step=0.1)
n_samples = st.number_input("Initial Monte Carlo Samples", value=10000, min_value=1000, step=1000)

# Subsampling and truncation settings
st.markdown("### First-Level Subsampling (From Base Distribution)")
subsample_size = st.number_input("Subsample Size", value=100, min_value=1)
n_subsample_runs = st.number_input("Subsample Monte Carlo Runs", value=1000, min_value=100)
lower_thresh = st.number_input("Specific Deductible ($)", value=0.0, step=100.0)
upper_thresh = st.number_input("Excess Layer Level ($)", value=50000.0, step=100.0)
